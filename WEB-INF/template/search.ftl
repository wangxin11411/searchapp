...
<script src="${(storeConfiguration.stageJsServer)!}/dist/search.bundle.js"></script>
<script>
    var prop4 = <#if ((searchObj.header.isNotContains)?? && searchObj.header.isNotContains)  &&  (!(searchObj.content.prodInfo.products)?? || (searchObj.content.prodInfo.products)?? && searchObj.content.prodInfo.products?size = 0)>"站内搜索失败页面"<#else>"站内搜索成功页面"</#if>;
</script>
