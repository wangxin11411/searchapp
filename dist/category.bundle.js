...
/***/ function(module, exports, __webpack_require__) {
	...
	if(isBwSec){
	    __webpack_require__(11).getShopGoods();
	}
	setTimeout(function(){
	    __webpack_require__(4).getGoods("三级列表页");
	},0);
	...
	$.get(
	    ...
	    function(data){
	        if (data.lst && data.lst.length > 0) {
	            ...
	            $("#prdRight-2").append('<div class="prd-right-normal"><h3 class="hd">一周销量排行榜</h3><ul class="sell-product" id="bigD_weekTop">'+listTpl+'</ul></div>');
	        }
	    }
	);
	...
	$.get(
	    ...
	    function(data){
	        if (data.lst && data.lst.length > 0) {
	            data.lst.splice(6,data.lst.length)
	            var listTpl = templateSimple.compile(tpl_normal)(data);
	            $("#prdRight-4").append('<div class="prd-right-normal"><h3 class="hd">搜了此类商品的用户还买了</h3><ul class="bd" id="bigD_liulan">'+listTpl+'</ul></div>');
	        }
	    }
	);
...
/***/ function(module, exports) {
	...
	$.ajax({
	    ...
	    success:function(data){
	        ...
	        $("#prdRight-5").append('<div class="prd-right-normal" id="dsp_advertisement">'+listTpl+'</div>');
	    }
	});
